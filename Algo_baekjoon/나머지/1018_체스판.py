...
# 몇개 고쳐야하는지 세기
def bruteforce(brd, visited, r, c):
    global N, M
    cnt = 0
    dq = deque()
    dq.append((r,c))
    while dq:
        ...
        for d in [(0,1), (1,0)]:
            nextR = curR + d[0]
            nextC = curC + d[1]
            if 0 <= nextR < N and 0 <= nextC < M and brd[curR][curC] == brd[nextR][nextC]:
...
for r in range(len(BRD)-8+1): # i: 0 1 2
    ...
    for c in range(len(BRD)-8+1):
        for k in range(8):
            sub_brd.append(BRD[r+k][c:c+8])
    visited = [[0 for _ in range(M)] for _ in range(N)]
    result = bruteforce(sub_brd, visited, r, c)
    if result < min_cnt:
        min_cnt = result
